...
if ($action === 'register') {
    ...
    try {
        $db->prepare('INSERT INTO students (admission_no,name,email,password) VALUES (?,?,?,?)
                      ON DUPLICATE KEY UPDATE email=VALUES(email),password=VALUES(password),name=VALUES(name)')
           ->execute([$adm,$name,$email,$hash]);
...
if ($action === 'login') {
    ...
    if ($student && $student['password'] && password_verify($pass, $student['password'])) {
        ...
        // Auto-detect passport file if not recorded in DB yet
        if (!$passport) {
            $passDir = __DIR__ . '/../passports/';
            $candidates = [
                $student['admission_no'] . '.jpg',
                str_replace('/', '', $student['admission_no']) . '.jpg',
                $student['id'] . '.jpg',
                str_replace(['/', '-', ' '], '', strtoupper($student['admission_no'])) . '.jpg',
            ];
            foreach ($candidates as $candidate) {
                if (file_exists($passDir . $candidate)) {
                    $passport = $candidate;
                    $db->prepare('UPDATE students SET passport=? WHERE id=?')
                       ->execute([$passport, $student['id']]);
                    break;
                }
            }
        }
        ...
        $_SESSION['student_passport'] = $passport;
        $mustReset = (bool)($student['must_reset_password'] ?? false);
        auditLog($db, $student['name'], 'Login', $student['admission_no'], 'Student logged in', 'student');
        jsonOut(['ok'=>true,'name'=>$student['name'],'adm'=>$student['admission_no'],'passport'=>$passport,'must_reset'=>$mustReset]);
    }
...
if ($action === 'logout') {
    if (!empty($_SESSION['student_name'])) {
        auditLog($db, $_SESSION['student_name'], 'Logout', $_SESSION['student_adm'] ?? '', 'Student logged out', 'student');
    }
    session_destroy();
    jsonOut(['ok'=>true]);
}
...
$studentAdm  = $_SESSION['student_adm']  ?? '';
...
if ($action === 'start_quiz') {
    ...
    if (!$existing) {
        ...
        auditLog($db, $studentName, 'Start Quiz', $subject['name'].' ('.$subject['code'].')', "Adm: $studentAdm", 'student');
    } else {
...
if ($action === 'submit_quiz') {
    ...
    auditLog($db, $studentName, 'Submit Quiz', '', "Score: $correctCount/$numServed ($pct%) Grade: $grade".($auto?' [auto]':''), 'student');
...
if ($action === 'change_password') {
    ...
    if (strlen($newPass) < 6) jsonOut(['ok'=>false,'msg'=>'Password must be at least 6 characters.']);
    $db->prepare('UPDATE students SET password=?, must_reset_password=0 WHERE id=?')
       ->execute([password_hash($newPass, PASSWORD_DEFAULT), $studentId]);
    auditLog($db, $studentName, 'Change Password', $studentAdm, 'Student changed own password', 'student');
    jsonOut(['ok'=>true,'msg'=>'Password changed successfully.']);
}
